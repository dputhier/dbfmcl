...
NOTE: THE VERSION IS CURRENTLY UNSTABLE AND ONLY FOR DEVELOPMENT PURPOSE.
...
```{r, installation, eval=TRUE, echo=TRUE, include=TRUE}
devtools::install_github("dputhier/dbfmcl")
library(dbfmcl)
```

## Installing MCL

Although an implementation of MCL exists for R (see the [MCL library](https://cran.r-project.org/web/packages/MCL/index.html)) we really encourage you to install the original MCL tool from S.M. van Dongen. Indeed, the R version is slower, not reproducible and produced very different results from the original version. 
...
reinstall <- function(){
  ...
  library(devtools)
  roxygen2::roxygenise(path_to_dbfmcl_git)
...
TODO : add some regular filtering steps.
...
Note that the result of the DBFMCL function is an S4 object (ClusterSet).
...
```{r}
showMethods(class="ClusterSet", where = "package:dbfmcl")
```
...
g_profiles_no_legend + dim_plot_clust
```
# Perform alternative cell classification
...
dbf_seurat@cell_order <- colnames(dbf_seurat@data)[hier_clustering$order]

g_profiles_hc <- plot_clust(dbf_seurat,
       average_only = TRUE,
       floor=-1, 
       ceil=1)
g_profiles_hc
